...
public final class RemindersPage extends VBox {
    ...
    public RemindersPage(ReminderService reminders, ReminderDeliveryService delivery, NotificationService notifications, Runnable openHistory) {
        ...
        for (int days : new int[]{90, 60, 30, 14, 7}) {
            ...
            check.setOnAction(event -> { reminders.setThresholdEnabled(days, check.isSelected()); refreshSummary(summary, reminders); });
        ...
        VBox pending = new VBox(12); pending.getStyleClass().addAll("card", "reminder-pending-card"); refreshPending(pending, reminders); getChildren().addAll(card, pending);
    }
