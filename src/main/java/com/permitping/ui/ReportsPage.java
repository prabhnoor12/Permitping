...
public final class ReportsPage extends VBox {
    private final DocumentService documents; private final AssignmentService assignments; private final ProfileService profiles; private final FileStorage files; private final NotificationService notifications; private final RequirementTemplateService templates; private final Label summary=new Label();
    public ReportsPage(DocumentService documents, AssignmentService assignments, ProfileService profiles, FileStorage files, NotificationService notifications) {
        this(documents, assignments, profiles, files, notifications, null);
    }
    public ReportsPage(DocumentService documents, AssignmentService assignments, ProfileService profiles, FileStorage files, NotificationService notifications, RequirementTemplateService templates) {
        this.documents=documents;this.assignments=assignments;this.profiles=profiles;this.files=files;this.notifications=notifications;this.templates=templates;getStyleClass().add("page-shell");setSpacing(18);setPadding(new Insets(30,34,30,34));getChildren().add(new PageHeader("Compliance reports","Export the current compliance picture for project reviews and record keeping."));
    ...
    private void exportAssignments(){if(assignments==null){notifications.error("Assignment storage is not configured.");return;}File file=choose("assignment-readiness.csv");if(file==null)return;try{var clearances=new ClearanceService(assignments,profiles,documents,files,Clock.systemDefaultZone(),templates).list();new ExportService().exportAssignmentsCsv(clearances,file.toPath());notifications.info("Report exported to "+file.getName());}catch(Exception ex){notifications.error("Report export failed: "+ex.getMessage());}}
