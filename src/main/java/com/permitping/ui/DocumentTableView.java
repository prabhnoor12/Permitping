...
public final class DocumentTableView extends TableView<Document> {
    ...
    private boolean matches(Document d){String q=search.getText()==null?"":search.getText().trim().toLowerCase();String hay=(d.name()+" "+d.type()+" "+holder(d)+" "+d.holder()+" "+(d.project()==null?"":d.project())).toLowerCase();if(!q.isBlank()&&!hay.contains(q))return false;ComplianceStatus s=d.status(Clock.systemDefaultZone());long days=d.daysUntilExpiry(Clock.systemDefaultZone());return switch(quickFilter){case "Expired"->s==ComplianceStatus.EXPIRED;case "Due this week"->days>=0&&days<=7;case "Due this month"->days>=0&&days<=30;case "Missing file"->!files.exists(d.filePath());case "By project"->projectFilter.equals("All projects")||projectFilter.equals(d.project());default->true;};}
