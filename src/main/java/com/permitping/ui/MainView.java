...
public final class MainView extends BorderPane {
    ...
    public MainView(DocumentService documents, RequirementTemplateService templates, ProfileService profiles, AssignmentService assignments, ReminderService reminders, DocumentVersionService versions, java.util.Set<Permission> permissions, AuthService auth, AuthUser authenticatedUser, Runnable onLogout) {
        ...
        subscriptions = profiles == null ? null : new NotificationSubscriptionService(new com.permitping.infrastructure.SqliteNotificationSubscriptionRepository(new com.permitping.infrastructure.Database(java.nio.file.Path.of(System.getProperty("user.dir"),"data","permitping.db"))), audit);
        ...
        var chaseKey = ChaseMessageProtector.fromEnvironment().orElse(null);
        chasing = assignments != null && profiles != null && templates != null && uploads != null && can(Permission.MANAGE_ASSIGNMENTS) && uploadPortal != null && chaseKey != null ? new DocumentChaseService(assignments, profiles, documents, templates, uploads, files, new com.permitping.infrastructure.SqliteDocumentChaseDeliveryRepository(new com.permitping.infrastructure.Database(java.nio.file.Path.of(System.getProperty("user.dir"),"data","permitping.db"))), chaseKey, new com.permitping.infrastructure.SendGridEmailSender(), new com.permitping.infrastructure.TwilioSmsSender(), subscriptions, audit, java.time.Clock.systemDefaultZone(), true) : null;
    ...
    private void showProfiles(){ if(!can(Permission.VIEW_PROFILES)){showDenied("profiles");return;} if(profiles==null){notifications.error("Profile storage is not configured.");return;} workspace.getChildren().setAll(new ProfilesPage(profiles,documents,assignments,notifications,subscriptions,can(Permission.MANAGE_PROFILES))); }
    ...
    private void showReports(){ if(!can(Permission.VIEW_REPORTS)){showDenied("reports");return;} workspace.getChildren().setAll(new ReportsPage(documents,assignments,profiles,files,notifications,templates)); }
    private void showAssignments(){ if(!can(Permission.MANAGE_ASSIGNMENTS)){showDenied("assignments");return;} if(assignments==null||profiles==null){notifications.error("Assignment storage is not configured.");return;} workspace.getChildren().setAll(new AssignmentsPage(assignments,profiles,clearance,projectNames(),notifications,uploads,this::showUploadInbox)); }
    private void showUploadInbox(){ if(uploads==null){notifications.error("Upload portal is not configured.");return;} workspace.getChildren().setAll(new UploadInboxPage(uploads,notifications,this::showAssignments)); }
    ...
    private void showPendingReminders(){
        ...
        if (reminders != null) notifications.reminders(reminders.pending());
    }
    ...
    private void stopUploadPortal(){ if(uploadPortal!=null)uploadPortal.close(); }
    private com.permitping.infrastructure.UploadPortalServer startUploadPortal(){ if(uploads==null||!"true".equalsIgnoreCase(System.getenv("PERMITPING_UPLOAD_ENABLED")))return null; try { int port=Integer.parseInt(System.getenv().getOrDefault("PERMITPING_UPLOAD_PORT","8765")); String bind=System.getenv().getOrDefault("PERMITPING_UPLOAD_BIND_ADDRESS","127.0.0.1"); var server=new com.permitping.infrastructure.UploadPortalServer(uploads,bind,port); server.start(); return server; } catch(Exception ex){ notifications.error("Upload portal could not start: "+(ex.getMessage()==null?"invalid configuration":ex.getMessage())); return null; } }
