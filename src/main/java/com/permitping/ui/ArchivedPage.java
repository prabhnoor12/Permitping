...
public final class ArchivedPage extends BorderPane {
    ...
    public ArchivedPage(DocumentService documents, NotificationService notifications) {
        this(documents, notifications, true);
    }
    public ArchivedPage(DocumentService documents, NotificationService notifications, boolean canManage) {
    ...
    private void restore(Document item){try{documents.restore(item.id());refresh();notifications.info("Document restored to the active register.");}catch(RuntimeException ex){notifications.error(ex.getMessage()==null?"Document could not be restored.":ex.getMessage());}}
