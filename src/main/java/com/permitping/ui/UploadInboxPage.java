...
public final class UploadInboxPage extends BorderPane {
    ...
    private void showDetails(UploadSubmission submission) { if (submission == null) { details.setText("Select an upload to review it."); return; } UploadRequest request = uploads.requestFor(submission); UploadVerification verification = uploads.verify(submission); UploadAnalysis analysis = uploads.analyze(submission); details.setText("Project: " + request.project() + "\nDocument type: " + request.documentType() + "\nFilename: " + submission.originalFilename() + "\nSize: " + submission.sizeBytes() + " bytes\nSubmitted: " + submission.submittedAt() + "\nStatus: Pending review\nAutomatic verification: " + verification.status() + "\n" + String.join("\n", verification.checks()) + "\nContent analysis\n" + String.join("\n", analysis.findings())); }
    private void accept(UploadSubmission submission) {
        ...
        UploadAnalysis analysis = uploads.analyze(submission); LocalDate suggested = analysis.suggestedExpiryDate(); DatePicker expires = new DatePicker(suggested == null ? LocalDate.now().plusYears(1) : suggested); TextField name = new TextField(request.documentType() + " - " + submission.originalFilename()); TextField reviewer = new TextField("PermitPing reviewer"); TextField notes = new TextField(); notes.setPromptText("Optional review notes"); Label expiryHelp = new Label(suggested == null ? "No expiry date was confidently found; confirm the document and enter one." : "Suggested from document text: " + suggested + ". Confirm it against the source before accepting."); expiryHelp.setWrapText(true); expiryHelp.getStyleClass().add("helper-text");
        GridPane grid = new GridPane(); grid.setHgap(14); grid.setVgap(12); grid.setPadding(new Insets(18)); grid.addRow(0, new Label("Document name"), name); grid.addRow(1, new Label("Expires on"), expires); grid.add(expiryHelp, 1, 2); grid.addRow(3, new Label("Reviewed by"), reviewer); grid.addRow(4, new Label("Notes"), notes); dialog.getDialogPane().setContent(grid);
