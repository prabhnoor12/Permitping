...
public final class FileStorage {
    ...
    public FileStorage(Path root) { this.root=root.toAbsolutePath().normalize(); }
    ...
    public boolean isManaged(String path) { if(path==null||path.isBlank())return false;try{Path candidate=Path.of(path).toAbsolutePath().normalize();if(!candidate.startsWith(root)||!Files.isRegularFile(candidate,LinkOption.NOFOLLOW_LINKS))return false;return candidate.toRealPath(LinkOption.NOFOLLOW_LINKS).startsWith(root.toRealPath(LinkOption.NOFOLLOW_LINKS));}catch(Exception e){return false;} }
    ...
    public void deleteManagedFile(Path path) throws IOException { if(path==null)return;Path normalized=path.toAbsolutePath().normalize();if(normalized.equals(root)||!normalized.startsWith(root))return;if(Files.isRegularFile(normalized,LinkOption.NOFOLLOW_LINKS))Files.deleteIfExists(normalized); }
