...
public final class SendGridEmailSender implements EmailSender {
    ...
    public SendGridEmailSender() { this(System.getenv("PERMITPING_SENDGRID_API_KEY"), System.getenv("PERMITPING_FROM_EMAIL")); }
    public SendGridEmailSender(String apiKey, String from) { this.apiKey=apiKey; this.from=from; }
    @Override public DeliveryResult send(OutgoingMessage message) {
        ...
        String body = "{\"personalizations\":[{\"to\":[{\"email\":\""+json(message.recipient())+"\"}]}],\"from\":{\"email\":\""+json(from)+"\"},\"subject\":\""+json(message.subject())+"\",\"content\":[{\"type\":\"text/plain\",\"value\":\""+json(message.body())+"\"}]}";
        try {
            HttpRequest request=HttpRequest.newBuilder(URI.create("https://api.sendgrid.com/v3/mail/send")).header("Authorization","Bearer "+apiKey).header("Content-Type","application/json").POST(HttpRequest.BodyPublishers.ofString(body, StandardCharsets.UTF_8)).build();
